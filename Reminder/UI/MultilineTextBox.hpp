...
public:
    MultiLineTextBox(float x, float y, float width, float height, const sf::Font& font, unsigned int fontSize)
        : _position(x, y), _size(width, height), _font(font), _fontSize(fontSize), _cursorPosition(0), _mouseLeftDown(false) {
...
private:
    ...
    void wrapText() 
    {
        ...
        _text.setString(_content);  // Обновляем отображаемое содержимое
    }
