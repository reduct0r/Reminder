...
namespace UI 
{
    ...
    void TextBox::handleEvent(sf::Event& event) 
    {
        ...
        if (event.type == sf::Event::KeyPressed && focus) 
        {
            ...
            //if (event.key.code == sf::Keyboard::Enter) // Очистка и получиние по нажатию Enter
            //{
        ...
        if (event.type == sf::Event::MouseButtonPressed) 
        {
            if (event.mouseButton.button == sf::Mouse::Left) 
            {
                if (getPinp.getSize() > 0)
                {
                    if (charWidth == 0)
                    {
                        charWidth = inpText.get().findCharacterPos(1).x - inpText.get().findCharacterPos(0).x;
                    }

                    unsigned int temp = (unsigned int)((event.mouseButton.x - posX) / charWidth);
                   
                    if (temp > getPinp.getSize()) 
                    { 
