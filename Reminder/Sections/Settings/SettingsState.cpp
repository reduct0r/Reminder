...
void SettingsState::InitButtons() {
  ...
  this->buttons["GITHUB_BTN"] =
      new ReminderUI::Button(winX / 1.05 - this->textures["GITHUB_ICON"].getSize().x * scale / 2.0,
                             winY / 1.05 - this->textures["GITHUB_ICON"].getSize().y * scale,
                             scale,
                             this->textures["GITHUB_ICON"],
                             this->textures["GITHUB_ICON"],
                             this->textures["GITHUB_ICON"]);
  this->buttons["APPLY_BTN"] =
      new ReminderUI::Button(winX / 2 - this->textures["APPLY_IDL"].getSize().x * scale / 2.0, winY / 1.2, scale,
                             this->textures["APPLY_IDL"], this->textures["APPLY_HVR"], this->textures["APPLY_HVR"]);

  if (!this->gfxSettings.fullscreen) {
    this->buttons["FULLSCREEN_BTN"] =
        new ReminderUI::Button(winX / 2 - this->textures["FULLSCREEN_MODE"].getSize().x * scale / 2.0,
                               winY / 2,
                               scale,
                               this->textures["FULLSCREEN_MODE"],
                               this->textures["FULLSCREEN_MODE2"],
                               this->textures["FULLSCREEN_MODE2"]);
  } else {
    this->buttons["FULLSCREEN_BTN"] =
        new ReminderUI::Button(winX / 2 - this->textures["WINDOW_MODE"].getSize().x * scale / 2.0,
                               winY / 2,
                               scale,
                               this->textures["WINDOW_MODE"],
                               this->textures["WINDOW_MODE2"],
                               this->textures["WINDOW_MODE2"]);
  }

  if (!this->gfxSettings.sound) {
    this->buttons["SOUND_BTN"] =
        new ReminderUI::Button(winX / 2 - this->textures["SOUND_ON"].getSize().x * scale / 2.0, winY / 3, scale,
                               this->textures["SOUND_ON"], this->textures["SOUND_ON2"], this->textures["SOUND_ON2"]);
  } else {
    this->buttons["SOUND_BTN"] =
        new ReminderUI::Button(winX / 2 - this->textures["SOUND_OFF"].getSize().x * scale / 2.0, winY / 3, scale,
...
void SettingsState::InitDropDownLists() {
  ...
  std::vector<std::string> l =
      {"Resolution", "960 x 540", "1024 x 576", "1152 x 648", "1280 x 720 (HD)", "1366 x 768", "1600 x 900",
       "1920 x 1080 (FHD)", "2560 x 1440 (QHD)", "3840 x 2160 (4K)", "7680 x 4320 (8K)"};
  ...
  this->dropDownLists["RESOLUTION"] = new ReminderUI::DropDownList(this->scale / 1.5,
                                                                   0,
                                                                   winX / 3.2
                                                                       - this->textures["DDL_MAIN"].getSize().x * scale
                                                                           / 2,
                                                                   0,
                                                                   &this->font,
                                                                   25,
...
void SettingsState::UpdateButtons() {
  ...
  if (this->buttons["SOUND_BTN"]->isPressed() and this->getKeyTime()) {
    if (this->gfxSettings.sound == 0) {
      ...
      this->buttons["SOUND_BTN"]->setNewTextures(this->textures["SOUND_OFF"],
                                                 this->textures["SOUND_OFF2"],
                                                 this->textures["SOUND_OFF2"]);
    } else {
      this->gfxSettings.sound = 0;
      this->buttons["SOUND_BTN"]->setNewTextures(this->textures["SOUND_ON"],
                                                 this->textures["SOUND_ON2"],
                                                 this->textures["SOUND_ON2"]);
    }
  }
  ...
  if (this->buttons["FULLSCREEN_BTN"]->isPressed() and this->getKeyTime()) {
    if (this->gfxSettings.fullscreen == 0) {
      ...
      this->buttons["FULLSCREEN_BTN"]->setNewTextures(this->textures["WINDOW_MODE"],
                                                      this->textures["WINDOW_MODE2"],
                                                      this->textures["WINDOW_MODE2"]);
    } else {
      this->gfxSettings.fullscreen = 0;
      this->buttons["FULLSCREEN_BTN"]->setNewTextures(this->textures["FULLSCREEN_MODE"],
                                                      this->textures["FULLSCREEN_MODE2"],
                                                      this->textures["FULLSCREEN_MODE2"]);
    }
  }
